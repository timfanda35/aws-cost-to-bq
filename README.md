...
```bash
gcloud run jobs create billing-loader-job \
  --image "${IMAGE}" \
  --region "${GCP_REGION:-us-central1}" \
  --service-account "${SERVICE_ACCOUNT}" \
  --set-env-vars "SOURCE_TYPE=s3,SOURCE_BUCKET=${SOURCE_BUCKET},SOURCE_PREFIX=${SOURCE_PREFIX:-},EXPORT_NAME=${EXPORT_NAME},GCS_BUCKET=${GCS_BUCKET},GCS_DESTINATION_PREFIX=${GCS_DESTINATION_PREFIX:-},BQ_PROJECT_ID=${BQ_PROJECT_ID},BQ_DATASET_ID=${BQ_DATASET_ID},BQ_TABLE_ID=${BQ_TABLE_ID},AWS_REGION=${AWS_REGION}" \
...
The service account needs `roles/run.invoker` on the Job.

```bash
gcloud scheduler jobs create http billing-loader-job-daily \
  --schedule "${CRON_SCHEDULE:-0 6 * * *}" \
  --uri "https://${GCP_REGION:-us-central1}-run.googleapis.com/apis/run.googleapis.com/v1/namespaces/${GCP_PROJECT_ID}/jobs/billing-loader-job:run" \
  --http-method POST \
...
```bash
# Run with default 3-period window
...
**5. Update the Job after a new image push:**

```bash
gcloud run jobs update billing-loader-job \
  --image "${IMAGE}" \
  --region "${GCP_REGION:-us-central1}"
```
